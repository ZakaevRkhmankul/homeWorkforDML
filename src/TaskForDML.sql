...
SELECT * from cars WHERE brand IN('Hyundai');
...
SELECT count(brand) AS count_of_chevrolets from cars  WHERE brand IN('Chevrolet');
...
SELECT * from cars WHERE model ILIKE 'a%';
...
SELECT sum(price) AS total_price from cars WHERE brand = 'Mersedes bens';
